...
(function ($) {
	...
	$.fn.calendar = function (options) {
		...
		_this.update = function (date) {
			...
			// var cols = Math.ceil((day + daysInMonth(date))/7 + 1);
			cols = 6;
			...
			/* set month head */
			var monthStr = months[date.getMonth()] + ' ' + date.getFullYear(); //dateToStr(date).replace(/-\d+$/, '');
			_this.find('.month').text(monthStr);
			_this.curMonth = dateToStr(date).replace(/-\d+$/, '');
		};
		...
		function updateTable(monthOffset) {
			var date = strToDate(_this.curMonth);
